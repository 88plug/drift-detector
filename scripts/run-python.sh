...
find_python() {
  ...
  for c in "${EIGHTYEIGHT_PYTHON:-}" "${PLUGIN_PYTHON:-}" "${DD_PYTHON:-}"; do
    _try "$c" && return 0
  done
...
PY="$(find_python)" || {
  echo "drift-detector: no Python >=${MIN_MAJOR}.${MIN_MINOR} found." >&2
  echo "  Set EIGHTYEIGHT_PYTHON=/path/to/python3 (or DD_PYTHON), or install Python 3." >&2
