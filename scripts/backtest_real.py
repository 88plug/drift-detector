...
_LR_STOP = frozenset([
    ...
    "do","did","have","has","at","by","from","as","can","will","would",
    "its","not","all","get","set","if","so","my","we","me","no","he",
    "she","they","them","their","our","us","up","out","an","am","re",
])

import re as _re2, statistics as _stat, collections as _coll
...
def _lr_features(entry: dict, profile: dict, sess_bursts: dict) -> list:
    ...
    burst_words = {w for w in _re2.findall(r'\b\w{3,}\b', ' '.join(turns).lower())
                   if w not in _LR_STOP}
    follow_words = {w for w in _re2.findall(r'\b\w{3,}\b', following.lower())
                    if w not in _LR_STOP}
    jaccard = (len(burst_words & follow_words) /
               max(len(burst_words | follow_words), 1))

    comp = _coll.defaultdict(float)
    import drift_score as _ds3
    for t in turns:
        s_res = _ds3.score_text(t, profile)
        for dim in ("hype", "meta", "verbosity", "hedges", "filler", "complexity", "length"):
            comp[dim] += s_res["stats"].get(f"{dim}_count", 0) * profile["weights"].get(dim, 1.0)
    for k in comp:
        comp[k] /= len(turns)
...
def _run_lr_backtest(corpus: list, profile: dict) -> dict:
    ...
    groups = _np.array([hash(e.get("session_id", "") or str(i)) % 100000
                        for i, e in enumerate(entries_used)])
    clf = _LR(C=0.1, max_iter=500)
...
def _r19_features(entry: dict, profile: dict, sess_bursts: dict,
                  sess_ok_scores: dict) -> list:
    ...
    bw = [w for w in _re2.findall(r'\b\w{3,}\b', burst_text.lower()) if w not in _LR_STOP]
    fw = [w for w in _re2.findall(r'\b\w{3,}\b', following.lower()) if w not in _LR_STOP]
    pw = [w for w in _re2.findall(r'\b\w{3,}\b', preceding.lower()) if w not in _LR_STOP]
    def _jac(a, b): sa, sb = set(a), set(b); return len(sa & sb) / max(len(sa | sb), 1)
    ...
    is_imp     = int(prec_first in {
        'fix','create','add','remove','update','write','build','implement',
        'refactor','move','delete','make','run','check','test','verify','debug',
        'do','go','get',
    })
    ...
    sess_n_corrs = sum(
        1 for e in priors
        if (_classify_reply(e.get("following_user_prompt") or "") if _classify_reply else "new_task")
           not in ("new_task", "approval", "continuation")
    )
    ...
    for t in turns:
        ...
        for dim in ("hype","meta","verbosity","hedges","filler","complexity","length"):
            comp[dim] += sr["stats"].get(f"{dim}_count", 0) * profile["weights"].get(dim, 1.0)
    for k in comp:
        comp[k] /= len(turns)

    score_var = _stat.variance(scores) if len(scores) > 1 else 0.0
    follow_is_corr = int(ns_follow in ("correction_substance","correction_style","frustration"))

    return [
        max_s, max_s / thr_, len(scores), score_var, max(scores) - min(scores),
        scores[-1], sum(1 for s in scores if s >= thr_) / len(scores),
        score_trend, score_slope,
        rate_k1, rate_k2, rate_k3, min(bidx, 50),
        sess_n_corrs, score_vs_sess,
        _jac(bw, fw), follow_wc, int(ns_follow == "new_task"),
        int(ns_follow in ("approval","continuation")), follow_is_corr,
        _jac(bw, pw), prec_wc, rr_ratio,
        int(ns_prec == "new_task"), int(ns_prec in ("approval","continuation")),
        int(ns_prec in ("correction_substance","correction_style","frustration")),
        is_imp, _jac(pw, fw),
        n_bullets, n_headers, n_sents, n_code, n_paras,
        vocab_div, burst_wc, n_sents / max(len(turns), 1),
        comp["hype"], comp["meta"], comp["verbosity"], comp["hedges"],
        comp["filler"], comp["complexity"], comp["length"],
    ]


def _run_dcd_backtest(corpus: list, profile: dict, dcd_steps: int = 8, ml_threshold: float = 0.58) -> dict:
    ...
    for tr, te in gkf.split(X, y, groups):
        clf = _ETC(n_estimators=500, max_depth=10, class_weight="balanced", random_state=42)
        clf.fit(X[tr], y[tr])
        probs[te] = clf.predict_proba(X[te])[:, 1]
    preds = (probs >= ET_THRESH).astype(int)

    # DCD: scan dcd_steps ahead for correction signal
    entry_id_to_idx = {e["id"]: i for i, e in enumerate(entries_used)}
...
def main():
    ...
    ap.add_argument("--dcd", action="store_true",
                    help="R19: ExtraTree 43-feature ML + multi-step DCD (steps=8, t=0.58)")
    ap.add_argument("--dcd-steps", type=int, default=8, help="DCD lookahead steps (default 8)")
    ap.add_argument("--dcd-threshold", type=float, default=0.58, help="ML threshold for DCD path (default 0.58)")
    args = ap.parse_args()
    ...
    if args.dcd:
        out = _run_dcd_backtest(corpus, profile,
                                dcd_steps=args.dcd_steps,
                                ml_threshold=args.dcd_threshold)
        print(json.dumps(out, indent=2))
        return
