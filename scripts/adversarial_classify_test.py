...
CASES = [
    ...
    ("i just logged into github to check the issue",       "correction", "FP-1",  "new-task via 'i just logged into'"),
    ("i just logged into the dashboard everything looks normal", "correction", "FP-2", "status update via 'i just logged into'"),
    ("so its all working, ready to ship now",              "correction", "FP-3",  "'so its all working' fires even with positive continuation"),
    ("so its all working and im happy with the result",    "correction", "FP-4",  "'so its all working' fires in approval context"),
    ("we rebooted already, everything looks good now",     "correction", "FP-5",  "'we rebooted already' fires in status context"),
    ("simulate some users for the load test setup",        "correction", "FP-6",  "'simulate some user' fires in new-task context"),
    ("simulate some user traffic to warm up the cache",    "correction", "FP-7",  "'simulate some user' fires in new-task context"),
    ("ok hot shot, what should we build next",             "correction", "FP-8",  "'ok hot shot' fires even in new-task opener"),
    ("complete manually if that is easier for you",        "correction", "FP-9",  "'complete manually' fires as option offer"),
    ("i dont see this being an issue at all",              "correction", "FP-10", "'i dont see this' fires in problem-denial"),
    ("i don't see this as a problem right now",            "correction", "FP-11", "'i don't see this' fires in problem-denial"),
    ("its easier said than done but worth trying",         "correction", "FP-12", "'its easier' fires in idiomatic phrase — LATENT FP"),
    ("could not establish connection to my wifi router",   "correction", "FP-13", "'could not establish connection' fires on unrelated error"),
    ("p360ultra is the model name fyi",                    "correction", "FP-14", "'p360ultra' fires on any mention of device name"),
    ("its easier to just hardcode it for now",             "correction", "FP-18", "'its easier' fires on shortcut proposal"),
    ("go ahead back to you on the auth piece",             "correction", "FP-19", "'go ahead back to you' fires on task handback"),
    ...
    ("you've gone completely off track here",              "ok",  "FN-1",  "off track = correction; 'stay on track' in vocab but reversed form missed"),
    ("your answer has regressed since yesterday",          "ok",  "FN-2",  "regression claim missed"),
    ("thats exactly backwards from what i need",           "ok",  "FN-3",  "contradiction missed"),
    ("the output doesn't match the requirements at all",   "ok",  "FN-4",  "mismatch claim missed"),
    ("youre going in circles same mistake again",          "ok",  "FN-5",  "repeated-error complaint missed"),
    ("that wasnt even close to what i asked for",          "ok",  "FN-6",  "strong mismatch missed"),
    ("you clearly didnt read my instructions",             "ok",  "FN-7",  "accusation of ignoring instructions missed"),
    ("this solution breaks the existing behavior",         "ok",  "FN-8",  "regression claim ('breaks existing') missed"),
    ("youre solving the wrong problem entirely",           "ok",  "FN-9",  "'wrong problem' not in vocab, only 'wrong approach/direction'"),
    ("the logic is inverted flip the condition",           "ok",  "FN-10", "direct inversion correction missed"),
    ("the context you used is stale",                      "ok",  "FN-12", "stale context correction missed"),
    ("youre overcomplicating this massively",              "ok",  "FN-13", "overcomplication complaint missed"),
    ("that code path was removed in v2",                   "ok",  "FN-15", "factual correction (removed path) missed"),
    ...
    ("you missed the point",        "correction", "EDGE-7", "'you missed' fires via 'you missed'"),
